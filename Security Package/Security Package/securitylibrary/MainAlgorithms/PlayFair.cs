...
namespace SecurityLibrary
{
    public class PlayFair : ICryptographicTechnique<string, string>
    {
        ...
        {
            // Define a 5x5 matrix of strings
            plainText += "f";
            ...
            for (int ias = 1; ias < plainText.Length - 1; ias += 2)
            {
                string wordPair = $"{plainText[ias]}{plainText[ias + 1]}";

                if (counts.ContainsKey(wordPair))
                {
                    counts[wordPair]++;
                }
                else
            ...
            Dictionary<string, int> sortedCounts = counts.OrderByDescending(kv => kv.Value).ToDictionary(kv => kv.Key, kv => kv.Value);
            Console.WriteLine(sortedCounts.Count);
            ...
            string[] specificWordPairs = { "TH", "HE", "AN", "IN", "ER", "ON", "RE", "ED", "ND", "HA", "AT", "EN", "ES", "OF", "NT", "EA", "TI", "TO", "IO", "LE", "IS", "OU", "AR", "AS", "DE", "RT", "VE" };
            //  specificWordPairs.Reverse();
            int uu = 0;
            foreach (var kvp in sortedCounts.Keys.ToList())
            {
            ...
            foreach (var kvp in sortedCounts.Keys.ToList())
            {
                op++;
                sortedCounts[kvp] = uu;
                if (uu == specificWordPairs.Length - 1)
                {

                }
                else if (op != 0 && op % 8 == 0 && uu < specificWordPairs.Length)
                {
                    uu++;
                }
            ...
            {
                string wordPair = $"{plainText[ias]}{plainText[ias + 1]}";
                text += specificWordPairs[sortedCounts[wordPair]];
